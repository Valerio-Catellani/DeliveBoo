...
<section class="container py-5">
    ...
    @if ($dishes && count($dishes) > 0)
        <table class="table table-bordered table-striped text-left hype-unselectable">
            <thead>
                <tr>
                        ...
                        <tr>
                            <td class="align-middle d-none d-xl-table-cell {{ $dish->visible ? '' : 'opacity-50' }}">
                                @if (isset($dish->image) && strpos($dish->image, 'http') === 0)
                                    <img src="{{ $dish->image }}" class="img-fluid" alt="dish image" style="height: 200px; width:200px">
                                @else
